...
class ImageCollection():
    ...
    def name(self):
        if not self._name:
            if self.type == CollectionType.PAYLOAD:
                ...
                self._name = f'{is_info["metadata"]["namespace"]}/{is_info["metadata"]["name"]}'
        return self._name
    ...
    def get_payload_images(self):
        ...
        for entry in payload_info['references']['spec']['tags']:
            ...
            commit = entry.get('annotations', {}).get('io.openshift.build.commit.id', '')
            repo = entry.get('annotations', {}).get('io.openshift.build.source-location', '')
            images.update({name: Image(name=name, pullspec=pullspec, commit=commit, repo=repo)})
        return images
